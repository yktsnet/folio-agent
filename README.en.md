...
If your CI can't reach the Zenn articles directory (`articlesDir`), you can fall back to a snapshot pre-generated with `folio-agent-sync-zenn`. See [Usage](#usage--api) for details.
...
**Build time**: `folio-agent-ingest` globs over the consumer site's `dist/` and `knowledge/` (plus Zenn articles, optionally — or a snapshot pre-generated by `folio-agent-sync-zenn`), converts HTML to text, counts tokens, and writes `knowledge.json`.
...
For environments where CI can't reach `articlesDir` (e.g. a private repo holding the articles), specify `zennSnapshotPath` as a sibling key of `zenn`. When `articlesDir` doesn't exist, ingest falls back to the JSON at this path:
...
Generate the snapshot with `folio-agent-sync-zenn` and commit it to the repo:

```bash
npx folio-agent-sync-zenn folio-agent.config.json zenn-snapshot.json
```

**No warning is printed when this fallback happens.** A warning is only emitted when neither `articlesDir` nor `zennSnapshotPath` is present, so if the snapshot goes stale, the build keeps succeeding while the knowledge silently goes out of date. Re-run `folio-agent-sync-zenn` periodically (e.g. in CI), or add your own staleness check.
...
- Build-time knowledge ingestion (URL glob selection + supplementary Markdown + Zenn articles, or a snapshot of them)
