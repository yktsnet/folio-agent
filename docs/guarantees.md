...
- `message`が1000字を超える場合、400を返す — 欠落（テスト未着地、[[18]]で追加予定）
- `CF-Connecting-IP`ヘッダが無いリクエストでも例外を投げず処理を継続する — 欠落（テスト未着地、[[18]]で追加予定）
...
- `DEFAULT_RATE_LIMIT_CONFIG`の実値は`{ shortWindowMinutes: 10, shortWindowMax: 6, longWindowHours: 12, longWindowMax: 12 }`である — 欠落（テスト未着地、[[18]]で追加予定）
...
- `<config.json> <output.json>`を受け取り、config.jsonを読んでknowledgeを生成し、output.jsonにJSONとして書き出す — 欠落（プロセスとしてのE2Eテスト未着地、[[18]]で追加予定）
- 引数が欠けている場合、`process.exitCode = 1`を設定し標準エラーに使い方を出力する — 欠落（同上）
...
- ウィザードの回答一式から、config json・テーマCSS・APIルート雛形・`.dev.vars`・`.gitignore`が実際にファイルシステム上へ整合した内容で生成される — 欠落（プロセスとしてのE2Eテスト未着地、[[18]]で追加予定。個々の生成ロジック関数は`test/init/questions.test.ts`・`test/init/writers.test.ts`・`test/init/cli.test.ts`でユニットレベルにカバー済み）
